...
def get_recommend_result(game_count: int, games: 'list[(int, int, int)]') -> dict:
    ...
    played_game = [i for i in games if i[1]]
    played_game_count = len(played_game)
    played_games_by_playtime = [i[0] for i in sorted(played_game, key=lambda x: -x[1])][:20]
    played_games_by_recent = [i[0] for i in sorted(played_game, key=lambda x: -x[2])][:20]
